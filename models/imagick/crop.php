...
abstract class Crop
{
    ...
    public function init($imagePath, $config)
    {
     ...
     */
    public function resizeAndCrop($targetWidth, $targetHeight, $targetQuality)
    {

        if (empty($targetWidth)){
            $targetWidth = $this->baseDimension['width'];
        }
        
        if (empty($targetHeight)){
            $targetHeight = $this->baseDimension['height'];
        }
        ...
        $this->originalImage->resizeImage($crop['width'], $crop['height'], \Imagick::FILTER_SINC, 1);
